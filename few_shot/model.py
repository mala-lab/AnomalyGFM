...
class Model_fine_tuning(nn.Module):
    def __init__(self, n_in_1, n_in_2, n_h, activation, negsamp_round, readout):
        super(Model_fine_tuning, self).__init__()
    ...
    def forward(self, seq1, adj, raw_adj, normal_prompt, abnormal_prompt, sparse=False):
        ...
        # normal_prompt = torch.squeeze(self.prompt(normal_prompt))
        # abnormal_prompt = torch.squeeze(self.prompt(abnormal_prompt))
        # residual feature
...
class Model_ablation(nn.Module):
    def __init__(self, n_in_1, n_in_2, n_h, activation, negsamp_round, readout):
        super(Model_ablation, self).__init__()
        self.read_mode = readout
        self.fc_map = nn.Linear(n_in_1, n_in_2, bias=False)
