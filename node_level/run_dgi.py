...
parser.add_argument('--dataset_test', type=str,
                    default='yelp')  # BlogCatalog_random  yelp  weibo_random reddit_random
...
optimiser = torch.optim.Adam(model_dgi.parameters(), lr=args.lr, weight_decay=args.weight_decay)
#
...
with tqdm(total=args.num_epoch) as pbar:
    ...
    for epoch in range(args.num_epoch):
        ...
        # DGI
        random_feat_train_indices = torch.randperm(feat_train.size(0))
        shuf_ftrain = feat_train[random_feat_train_indices]

        lbl_1 = torch.ones(nb_nodes)
        lbl_2 = torch.zeros(nb_nodes)
        lbl = torch.cat((lbl_1, lbl_2), 0)

        logits, logit_residual, emb, emb_residual, normal_prompt, abnormal_prompt = model_dgi(feat_train, shuf_ftrain,
                                                                                              adj_train,
                                                                                              raw_adj_train,
                                                                                              normal_prompt,
                                                                                              abnormal_prompt)
        ...
        # DGI Loss Function
        loss_dgi = torch.mean(b_xent(torch.squeeze(logits), lbl))
        ...
        # loss_alignment = torch.mean(dif_normal) + torch.mean(dif_abnormal)
        loss_alignment = torch.mean(dif_normal)

        # if epoch < 200:
        #     loss = loss_dgi
        # else:
        #     loss = loss_dgi + loss_alignment
        ...
        print("Epoch:", '%04d' % (epoch), "loss_dgi =", "{:.5f}".format(loss_dgi.item()))
        ...
        if epoch % 10 == 0:
            ...
            abnormal_prompt = F.normalize(torch.unsqueeze(abnormal_prompt, 1), p=2, dim=1)
            normal_prompt = F.normalize(torch.unsqueeze(normal_prompt, 1), p=2, dim=1)

            emb_residual_val = F.normalize(torch.squeeze(emb_residual_val), p=2, dim=1)
            score_normal = torch.mm(emb_residual_val, normal_prompt)
            score_abnormal = torch.mm(emb_residual_val, abnormal_prompt)
        ...
        if epoch % 10 == 0:
            ...
            normal_prompt = torch.randn(args.embedding_dim)
            abnormal_prompt = torch.randn(args.embedding_dim)
            logits_test, logits_test_residual, emb_test, emb_residual_test, normal_prompt_test, abnormal_prompt_test = model_dgi(
                feat_test,
                feat_test,
            ...
            # Using Distance
            # score_normal = torch.mm(emb_residual_test, normal_prompt_test)
            # score_abnormal = torch.mm(emb_residual_test, abnormal_prompt_test)

            # Obtain the anomaly score on the test data
            ...
            # abnormal_prompt_test = torch.squeeze(F.normalize(torch.unsqueeze(abnormal_prompt_test, 1), p=2, dim=1))
            # normal_prompt_test = torch.squeeze(F.normalize(torch.unsqueeze(normal_prompt_test, 1), p=2, dim=1))
            # # Residual on test normalize
            # emb_residual_test = F.normalize(torch.squeeze(emb_residual_test), p=2, dim=1)

            # Obtain the anomaly score on the test data Using Corr
            # abnormal_prompt_test = abnormal_prompt_test / torch.norm(abnormal_prompt_test, dim=-1, keepdim=True)
            # normal_prompt_test = normal_prompt_test / torch.norm(normal_prompt_test, dim=-1, keepdim=True)
            abnormal_prompt_test = F.normalize(torch.unsqueeze(abnormal_prompt_test, 1), p=2, dim=1)
            normal_prompt_test = F.normalize(torch.unsqueeze(normal_prompt_test, 1), p=2, dim=1)
