...
parser.add_argument('--auc_test_r ounds', type=int, default=256)
...
with tqdm(total=args.num_epoch) as pbar:
    ...
    for epoch in range(args.num_epoch):
        ...
        if epoch % 10 == 0:
            ...
            #
            # emb_residual_val = torch.squeeze(emb_residual_val)

            # Obtain the anomaly score on the test data using distance
            # score_normal = torch.squeeze(
            #     torch.sqrt(torch.sum((emb_residual_val - torch.unsqueeze(normal_prompt, 0)) ** 2, dim=1)))
            # score_abnormal = torch.squeeze(
            #     torch.sqrt(torch.sum((emb_residual_val - torch.unsqueeze(abnormal_prompt, 0)) ** 2, dim=1)))
